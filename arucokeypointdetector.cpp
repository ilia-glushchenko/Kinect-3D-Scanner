	...
	)
{
	...
	if (settings->value("ARUCO_SETTINGS/GREEN_MARKER").toBool())
	{
		cv::Mat InImage_HSV, InImage_GRAY;
		cv::cvtColor(InImage, InImage_GRAY, CV_BGR2GRAY);
		cv::cvtColor(InImage, InImage_HSV, CV_BGR2HSV);

		int b_from = settings->value("ARUCO_SETTINGS/H_B_FROM").toInt();
		int b_to   = settings->value("ARUCO_SETTINGS/H_B_TO").toInt();
		int g_from = settings->value("ARUCO_SETTINGS/H_G_FROM").toInt();
		int g_to   = settings->value("ARUCO_SETTINGS/H_G_TO").toInt();
	...
	MDetector.setThresholdParams(
		settings->value("ARUCO_SETTINGS/THRESHOLD_FROM").toInt(),
		settings->value("ARUCO_SETTINGS/THRESHOLD_TO").toInt()
		);
	MDetector.setCornerRefinementMethod(aruco::MarkerDetector::LINES);
	MDetector.setWarpSize(
		(
		D[0].n() +
		settings->value("ARUCO_SETTINGS/WARP_SIZE_COEF_1").toFloat()
		) * settings->value("ARUCO_SETTINGS/WARP_SIZE_COEF_2").toFloat()
		);
	MDetector.setMinMaxSize(
		settings->value("ARUCO_SETTINGS/MINIMUM_MARKER_SIZE").toFloat(),
		settings->value("ARUCO_SETTINGS/MAXIMUM_MARKER_SIZE").toFloat()
		);

	float MarkerSize =
		settings->value("ARUCO_SETTINGS/MARKER_SIZE").toFloat();
	MDetector.detect(InImage, *Markers, CamParam, MarkerSize);

	if (settings->value("ARUCO_SETTINGS/DRAW_EACH_MARKER").toBool() ||
		settings->value("ARUCO_SETTINGS/DRAW_STREAM_MARKER").toBool())
	{
		for (int i = 0; i < Markers->size(); i++)
		{
			Markers->at(i).draw(InImage, cv::Scalar(0, 0, 255), 2);
			if (settings->value("ARUCO_SETTINGS/DRAW_CUBES").toBool())
				CvDrawingUtils::draw3dCube(InImage, Markers->at(i), CamParam);
		}

		if (settings->value("ARUCO_SETTINGS/DRAW_EACH_MARKER").toBool())
			cv::imshow(QString("marker %1").arg(rand()).toStdString(), InImage);
		if (settings->value("ARUCO_SETTINGS/DRAW_STREAM_MARKER").toBool())
			cv::imshow("markers", InImage);
		if (settings->value("ARUCO_SETTINGS/DRAW_STREAM_THRESH").toBool())
...
void ArUcoKeypointDetector::find_keypoints()
{
	...
	for (int i = 0; i < marker_matches.size(); i++)
	{
		...
		if (isnan<float>(point_cloud_ptr1.get()->at(x1, y1).z) == false &&
			isnan<float>(point_cloud_ptr2.get()->at(x2, y2).z) == false)
		{
			keypoint_point_cloud_ptr1.get()->width = keypoint_point_cloud_ptr1->points.size() + 1;
			keypoint_point_cloud_ptr1.get()->height = 1;
			keypoint_point_cloud_ptr1.get()->points.push_back(
				point_cloud_ptr1.get()->at(x1, y1));

			keypoint_point_cloud_ptr2.get()->width = keypoint_point_cloud_ptr2->points.size() + 1;
			keypoint_point_cloud_ptr2.get()->height = 1;
			keypoint_point_cloud_ptr2.get()->points.push_back(
				point_cloud_ptr2.get()->at(x2, y2));
		}
