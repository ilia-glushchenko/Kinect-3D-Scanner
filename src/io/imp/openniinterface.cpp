...
{
    boost::filesystem::path calib_mat_path = QFileInfo(settings->fileName()).canonicalFilePath().toStdString();
    calib_mat_path = calib_mat_path.parent_path()
        / settings->value("PROJECT_SETTINGS/CALIB_DATA_FOLDER").toString().toStdString()
        / configs.value("OPENNI_SETTINGS/CALIB_MATRIX_NAME").toString().toStdString();
        
    boost::filesystem::path dist_coeffs_path = QFileInfo(settings->fileName()).canonicalFilePath().toStdString();
    dist_coeffs_path = dist_coeffs_path.parent_path()
        / settings->value("PROJECT_SETTINGS/CALIB_DATA_FOLDER").toString().toStdString()
        / configs.value("OPENNI_SETTINGS/DIST_COEFF_NAME").toString().toStdString();

    if (boost::filesystem::exists(calib_mat_path) && boost::filesystem::exists(dist_coeffs_path)) 
    {
        calib_matrix = (CvMat*)cvLoad(calib_mat_path.string().c_str());
        dist_coeffs = (CvMat*)cvLoad(dist_coeffs_path.string().c_str());
    } 
    else 
    {
