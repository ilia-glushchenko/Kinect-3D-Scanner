...
public:
	...
	EdgeBalancer(	Iter & begin_iterator,
					Iter & end_iterator,
...
private:
	...
	struct Edge
	{
		...
		Edge(const int & index_, Iter & it_) :
			index(index_),
			it(it_),
	...
	void init_edges(const uint & begin_edge_index = 0)
	{
		...
		while (frame_it != end_it)
		{
			...
			if (tail_insert_needed || (std::next(frame_it) == end_it))
			{
				int pass = 0;
			}
	...
	void calculate_abs_deviations()
	{
		average_metric = std::accumulate(edges.begin(), edges.end(), double(),
			[](const double & sum, const Edge & edge) { return sum + edge.metric; }) / edges.size();

		calculate_abs_deviations(average_metric);

		average_abs_deviation = std::accumulate(edges.begin(), edges.end(), double(),
			[](const double & sum, const Edge & edge) { return sum + edge.abs_deviation; }) / edges.size();
	}
