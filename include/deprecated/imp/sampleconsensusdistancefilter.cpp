    ...
    QObject* parent)
{
    ...
    inlier_threshold = settings->value("SAC_SETTINGS/INLIER_THRESHOLD").toFloat();
    max_iter = settings->value("SAC_SETTINGS/MAX_ITERATIONS").toInt();
    distance_threshold = settings->value("SAC_DISTANCE_FILTER/DISTANCE_THRESHOLD").toFloat();
    triplets_size = settings->value("SAC_DISTANCE_FILTER/TRIPLETS_SIZE").toInt();
    allow_zero_distances = settings->value("SAC_DISTANCE_FILTER/ALLOW_ZERO_DISTANCE").toBool();
}
    ...
    PcdPtr target_point_cloud_ptr)
{
    ...
    svd.estimateRigidTransformation(*input_point_cloud_ptr.get(), *target_point_cloud_ptr.get(), correspondences_vector, transformation);
    ...
    PcdPtr target_point_cloud_ptr)
{
    ...
    for (int i = 0; i < _src_correspondences_vector.size(); i++) {
        ...
        float _distance = sqrtf(
            powf(input_point.x - target_point.x, 2) + powf(input_point.y - target_point.y, 2) + powf(input_point.z - target_point.z, 2));
...
void SampleConsensusDistanceFilter::fill_dst_point_clouds()
{
    ...
    for (int i = 0; i < _dst_correspondences_vector->size(); i++) {
        ...
        PointType input_point = _src_input_point_cloud_ptr.get()->points[_dst_correspondences_vector->at(i).index_query];
        PointType target_point = _src_target_point_cloud_ptr.get()->points[_dst_correspondences_vector->at(i).index_match];
...
void SampleConsensusDistanceFilter::iterative_rejection()
{
    ...
    do {
        ...
        Eigen::Matrix4f transformation = Eigen::Matrix4f::Identity();
        pcl::registration::TransformationEstimationSVD<PointType, PointType> svd;
        svd.estimateRigidTransformation(*input_point_cloud_ptr.get(), *target_point_cloud_ptr.get(), _src_correspondences_vector, transformation);
        pcl::transformPointCloud(
            *input_point_cloud_ptr.get(),
            *input_point_cloud_ptr.get(),
            transformation);
