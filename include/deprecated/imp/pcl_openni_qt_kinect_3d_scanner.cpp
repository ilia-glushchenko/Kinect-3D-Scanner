...
void PCL_OpenNI_Qt_Kinect_3d_Scanner::read_data()
{
    ...
    for (int i = from; i <= to; i += step) {
        ...
        QString filename_pattern = settings->value("READING_PATTERNS/POINT_CLOUD_IMAGE_NAME").toString();
    ...
    PcdPtr src_point_cloud_ptr)
{
    ...
    int d = settings->value(
                ...
                .toInt();
    double sigma_color = settings->value(
                                     "OPENCV_BILATERAL_FILTER_SETTINGS/SIGMA_COLOR")
                             .toDouble();
    double sigma_space = settings->value(
    ...
    PcdPtr keypoint_cloud_ptr2)
{
    ...
    if (keypoint_cloud_ptr1.get()->width > 0 && keypoint_cloud_ptr2.get()->width > 0) {
...
void PCL_OpenNI_Qt_Kinect_3d_Scanner::perform_sac_registration()
{
    ...
    if (!settings->value("CPU_TSDF_SETTINGS/ENABLE_IN_FINAL").toBool() && settings->value("SAC_SETTINGS/APPLY_IN_FINAL").toBool()) {
